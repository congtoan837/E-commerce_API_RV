...
import com.poly.ex.content.EString;
...
public class OrderService {
    ...
    public OrderResponse getDetailOrder(UUID orderId) {
    ...
    public OrderResponse cancelOrder(OrderRequest request) {
        Order order = orderRepository
                ...
                .filter(o -> EString.PENDING.getName().equals(o.getStatus()))
                .orElseThrow(() -> new AppException(ErrorCode.ORDER_NOT_FOUND));

        // Cập nhật trạng thái đơn hàng
        order.setStatus(EString.CANCELLED.getName());

        return orderMapper.toOrderResponse(orderRepository.save(order));
    }

    public void cancelUnpaidOrders() {
        List<Order> unpaidOrders = orderRepository.findByStatus(EString.PENDING.getName());
        unpaidOrders.stream()
                .filter(order ->
